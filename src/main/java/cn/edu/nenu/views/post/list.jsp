    ...
    <script>
        $(document).ready(function(){
            $("#btn_Create").click(function(){
                window.location.href="${ctx}/post/create";
            }) ;
            $(".cla_delete").bind("click",function () {
                if (confirm("确定要删除吗？")){
                    var id=$(this).attr("id");
                    window.location.href="${ctx}/post/delete/"+id;
                }
            });
            $(".cla_edit").bind("click",function () {
                window.location.href="${ctx}/post/update/${post.id}";
            })
            $("#btn_Delete").bind("click",function () {
                ...
                if(count<1){
                    ...
                    if (confirm("确定要删除所选项吗？")){
                        $("#listFrm").attr({action:"${ctx}/post/delete", method:"POST"})
                        $("#listFrm").submit();
                    }
                }
...
<div class="container">
    ...
    <table id="contentTable" class="table table-striped table-hover">
        <thead><tr>
            ...
            <th>管理</th></tr></thead>
        <tbody>
        <c:forEach items="${posts.content}" var="post" varStatus="idxStatus">
            <tr>
                <td><input type="checkbox" name="chkIds" value="${post.id}"></td>
                ...
                <td>${post.lastModifiedAt}</td>
                <td>${post.status}</td>
                <td>
                    <a href="${ctx}/post/update/${post.id}">编辑</a>
<%--                    <a href="#" class="cla_edit" id="${post.id}">编辑</a>--%>
                    |
                    <a href="#" class="cla_delete" id="${post.id}">删除</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    <tags:pagination page="${posts}" paginationSize="${PAGE_SIZE}"/>
    </form>
</div>
